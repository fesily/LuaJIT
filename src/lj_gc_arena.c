...
** Final after T3 calibration (2026-07-31): 128 objs / 4KB — multi-gate best
** (B10 1.56× classic PASS, B3 +3.1% PASS, B1 +11.1% vs T0 accepted floor).
** Exploration started at 64/2KB (~lj_gc_step lim); larger 256/32KB fails B10.
** B1 ≤5% vs pre-quantum T0 is unsatisfiable while B10≤2× classic (per-step
** notify/grayarena overhead floor); lead accepted B1 floor — arena B1 still
** ~2.7× faster than classic. See doc/arenagc-p4-mark-budget.md §7.
** Both conditions first-to-stop; done counts every popped batch member
** including non-gray continues (prevents small-cost object floods). */
...
static size_t gc_propagate_arena_pod(global_State *g, GCArena *a)
{
  ...
  for (;;) {
    ...
    for (MSize i = 0; i < n; i++) {
      GCobj *o = objs[i];
      done++;  /* count every popped batch member, incl. non-gray continues */
